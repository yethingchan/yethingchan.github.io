...
// 分页见 [[MyBatis-Plus/09-分页插件]]：IPage<T> selectPage(IPage<T>, Wrapper<T>)
```
```java
...
```
> 简单等值好用，复杂（like/范围/in）还是老老实实上 [[MyBatis-Plus/05-QueryWrapper详解]]。
...
```
MP 提供**防全表更新/删除拦截器**（[[MyBatis-Plus/15-企业级实战与避坑]] 详述）：
```java
@Bean
...
> 下一步：[[MyBatis-Plus/04-IServer与ServiceImpl]] 看比 BaseMapper 更顺手的 Service 层。
