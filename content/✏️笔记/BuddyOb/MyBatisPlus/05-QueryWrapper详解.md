...
> 上接：[[04-IServer与ServiceImpl]]
> **Wrapper（条件构造器）是 MP 的灵魂**。本章把所有查询条件方法一次讲透，并附"速查表"。更新看 [[06-UpdateWrapper详解]]，类型安全列看 [[07-LambdaWrapper与列安全]]。
...
```
> `inSql` / `notInSql` 第二参是**子查询字符串**，注意若拼用户输入有注入风险（用 [[08-动态条件与自定义SQL]] 的 `apply` 占位更安全）。
...
```
> 全部方法都带 **`boolean condition` 重载**（如 `eq(boolean, col, val)`），`condition=false` 时该条件**整条跳过**——这就是 [[08-动态条件与自定义SQL]] 的"动态条件"基础。
...
> 下一步：[[06-UpdateWrapper详解]] 看"更新"怎么用 Wrapper 不带实体。
